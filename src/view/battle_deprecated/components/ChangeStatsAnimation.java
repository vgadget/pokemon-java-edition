    ...
    public ChangeStatsAnimation(Dimension frameDimension, int locationX, int locationY) throws IOException {
        ...
        for (int i = 0; i < NUMBER_OF_SPRITES; i++) {
            ...
            if (frameDimension.equals(Dimensions.frameDimension720p)) {
                ...
                spriteUP[i] = utilities.image.ImageUtil.resizeProportional(spriteUP[i], 2);
                spriteDown[i] = utilities.image.ImageUtil.resizeProportional(spriteDown[i], 2);

            } else if (frameDimension.equals(Dimensions.frameDimension1080p)) {
                spriteUP[i] = utilities.image.ImageUtil.resizeProportional(spriteUP[i], 4);
                spriteDown[i] = utilities.image.ImageUtil.resizeProportional(spriteDown[i], 4);
            }
        }

        if (frameDimension.equals(Dimensions.frameDimension720p)) {

            this.locationXUp = locationX - 25;
